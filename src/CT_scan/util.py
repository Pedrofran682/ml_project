...
def irc2xyz(coord_irc, origin_xyz, vxSize_xyz, direction_a):
    ...
    coords_xyz = (direction_a @ (cri_a * vxSize_xyz)) + origin_a
    return XyzTuple(*coords_x)

def xyz2irc(coord_xyz, origin_xyz, vxSize_xyz, direction_a):
    origin_a = np.array(origin_xyz)
    vxSize_a = np.array(vxSize_xyz)
    coord_a = np.array(coord_xyz)
    cria_a = ((coord_xyz - origin_a) @ np.linalg.inv(direction_a)) / vxSize_a
    cria_a = np.round(cria_a)
    return IrcTuple(int(cria_a[2]), int(cria_a[1]), int(cria_a[0]))
