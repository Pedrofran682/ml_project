...
#         self.label = fileColumns.pop()
#         self.columns = fileColumns
#         self.data_iterator = self._get_records_iterator(file_path)
#         # self.dataScaler = dataScaler
#         # self.processor_fn = processor_fn
#         # Determine file size or number of lines/records if needed for sharding
#         # self.num_records = self._get_num_records(file_path) # Example helper
...
#     def _get_records_iterator(self, file_path):
#         # Replace this with logic to iterate over your specific data
#         for batch in pq.ParquetFile(file_path).iter_batches(1):
#             yield (batch.select(self.columns).to_pandas().to_numpy()[0][0],
#                    batch.select([self.label]).to_pandas().to_numpy()[0][0])
            
#     def __getitem__(self, idx):
#         features, labels= next(self.data_iterator)
